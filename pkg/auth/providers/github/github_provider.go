...
func (g *Provider) LoginUser(host string, githubCredential *apiv3.GithubLogin, config *apiv3.GithubConfig, test bool) (apiv3.Principal, []apiv3.Principal, string, error) {
	...
	name := cmp.Or(githubCredential.ConfigName, ProviderName)
	if config == nil {
		logrus.Debugf("Loading GitHub config %s", name)
		config, err = g.getConfig(name)
		if err != nil {
	...
	userPrincipal = g.toPrincipal(userType, user, name, nil)
	...
	for _, orgAcct := range orgAccts {
		groupPrincipal := g.toPrincipal(orgType, orgAcct, name, nil)
	...
	for _, teamAcct := range teamAccts {
		groupPrincipal := g.toPrincipal(teamType, teamAcct, name, nil)
		groupPrincipal.MemberOf = true
		groupPrincipals = append(groupPrincipals, groupPrincipal)
	}
...
func (g *Provider) RefetchGroupPrincipals(principalID, secret string) ([]apiv3.Principal, error) {
	...
	config, err := g.getConfig(provider)
	if err != nil {
		return nil, err
	}
	...
	for _, orgAcct := range orgAccts {
		groupPrincipal := g.toPrincipal(orgType, orgAcct, provider, nil)
		groupPrincipal.MemberOf = true
		groupPrincipals = append(groupPrincipals, groupPrincipal)
	}
	...
	for _, teamAcct := range teamAccts {
		groupPrincipal := g.toPrincipal(teamType, teamAcct, provider, nil)
		groupPrincipal.MemberOf = true
		groupPrincipals = append(groupPrincipals, groupPrincipal)
	}
...
func (g *Provider) SearchPrincipals(searchKey, principalType string, token accessor.TokenAccessor) ([]apiv3.Principal, error) {
	...
	config, err := g.getConfig(token.GetAuthProvider())
	if err != nil {
	...
	for _, acct := range accts {
		...
		p := g.toPrincipal(pType, acct, token.GetAuthProvider(), token)
	...
	if principalType == "" || principalType == "group" {
		...
		for _, acct := range teamAccts {
			p := g.toPrincipal(teamType, acct, config.GetName(), token)
			principals = append(principals, p)
		}
	}
...
func (g *Provider) GetPrincipal(principalID string, token accessor.TokenAccessor) (apiv3.Principal, error) {
	...
	princ := g.toPrincipal(principalType, acct, config.GetName(), token)
	return princ, nil
}

func (g *Provider) toPrincipal(principalType string, acct common.GitHubAccount, providerName string, token accessor.TokenAccessor) apiv3.Principal {
	...
	princ := apiv3.Principal{
		ObjectMeta:     metav1.ObjectMeta{Name: providerName + "_" + principalType + "://" + strconv.Itoa(acct.ID)},
