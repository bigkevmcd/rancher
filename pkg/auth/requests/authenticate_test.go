...
func TestAuthenticateWithAccessToken(t *testing.T) {
	...
	testIndexer := cache.NewIndexer(cache.MetaNamespaceKeyFunc, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
	testIndexer.AddIndexers(cache.Indexers{tokenKeyIndex: tokenKeyIndexer})
